  ...
  <div id="comment-<?php comment_ID(); ?>" class="comment-body">
    ...
    <div class="comment-meta">
      ...
      <?php if ( $rating && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) : ?>
        ...
        <div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="cs-star-rating" title="<?php echo sprintf( __( 'Rated %d out of 5', 'woocommerce' ), $rating ) ?>">
          <?php
            for ($i=0; $i < $rating; $i++) {
              echo '<i class="fa fa-star cs-star"></i>';
            }

            for ($i=0; $i < (5-$rating); $i++) {
              echo '<i class="fa fa-star cs-star-o"></i>';
            }
          ?>
        </div>
