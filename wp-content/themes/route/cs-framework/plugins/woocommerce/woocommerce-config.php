...
if ( ! function_exists( 'cs_woocommerce_loop_add_to_cart_link' ) ) {
  function cs_woocommerce_loop_add_to_cart_link( $link, $product ){
    ...
    return sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="button %s %s product_type_%s"><i class="fa fa-refresh fa-spin"></i><i class="fa fa-check"></i>%s</span></a>',
      esc_url( $product->add_to_cart_url() ),
      esc_attr( $product->id ),
      esc_attr( $product->get_sku() ),
      esc_attr( isset( $quantity ) ? $quantity : 1 ),
      esc_attr( cs_get_button_class( array( 'size' => 'xxs', 'type' => 'outlined', 'color' => 'outlined-accent' ) ) ),
