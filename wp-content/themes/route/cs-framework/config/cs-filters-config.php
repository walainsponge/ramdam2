...
if ( ! function_exists( 'cs_auto_excerpt_read_more_link' ) ) {
  function cs_auto_excerpt_read_more_link( $content ) {
    ...
    if ( cs_get_option( 'blog_auto_excerpt' ) === true && get_post_type() == 'post' && ! is_single() && ! is_search() && ! strpos( $content, 'cs-btn' ) ) {
...
if( is_gravityforms_activated() && ! function_exists( 'cs_gform_submit_class' ) ) {
  function cs_gform_submit_class( $button ){
    $button = str_replace( 'gform_next_button', cs_get_button_class() .' gform_next_button', $button );
    $button = str_replace( 'gform_previous_button', cs_get_button_class() .' gform_previous_button', $button );
    $button = str_replace( 'gform_button', cs_get_button_class() .' gform_button', $button );
    return $button;
  }
...
/**
 *
 * Yoast Seo Plugin disable Page Analysis
 *
 * @since 2.7.0
 * @version 1.0.0
 *
 */
if( ! function_exists( 'cs_disable_wpseo_use_page_analysis' ) ) {
  function cs_disable_wpseo_use_page_analysis() {
    return false;
  }
  add_filter( 'wpseo_use_page_analysis', 'cs_disable_wpseo_use_page_analysis' );
}
